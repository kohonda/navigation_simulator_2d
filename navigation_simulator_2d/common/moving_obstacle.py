...
class MovingObstacle(NamedTuple):
    ...
    def __str__(self) -> str:
        return "MovingObstacle((x,y,yaw)={}, (vx,vy,w)={}, shape={}, size={}, motion_model={})".format(
            [self.pos[0], self.pos[1], self.yaw], [self.linear_vel[0], self.linear_vel[1] , self.angular_vel], self.shape, self.size, self.motion_model)
        
    def __repr__(self)->str:
        return self.__str__()
    
    def __copy__(self):
        return MovingObstacle(self.pos, self.yaw , self.linear_vel, self.target_vel ,self.angular_vel, self.shape, self.size, self.motion_model)

    def __deepcopy__(self):
        return MovingObstacle(self.pos.copy(), self.yaw , self.linear_vel.copy(), self.target_vel , self.angular_vel, self.shape, self.size.copy(), self.motion_model)
    
    def draw_2d(self, base_map: np.ndarray, pose2index: Callable, meter2pixel: Callable, val: float)->np.ndarray:
        """
    ...
    def _social_force_model(self, target_vel: np.ndarray, obs_pos: np.ndarray)-> np.ndarray:
        """
    ...
    def _social_forced_motion(self, dt: float , obs_pos: np.ndarray) -> Tuple(np.ndarray, np.ndarray):
    ...
    def _reactive_stop_motion(self, prediction_horizon: int, dt: float, obs_pos: np.ndarray, obs_radius: float) -> Tuple(np.ndarray, float):
        """
        ...
        for i in range(prediction_horizon):
            new_pos, new_yaw = point_mass_prediction(pos, yaw, linear_vel, angular_vel, dt)
    ...
    def step(self, dt: float, obs_pos: np.ndarray = None, obs_radius: float = None)->'MovingObstacle':
        """
        ...
        if self.motion_model == MotionModel.POINT_MASS_MODEL:
            ...
            if obs_radius is None:
                raise ValueError("obs_radius must be set if you use reactive stop model")
            
            prediction_horizon = 3 # [steps]
            new_pos, new_yaw = self._reactive_stop_motion(prediction_horizon, dt, obs_pos, obs_radius)
