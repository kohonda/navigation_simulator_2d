...
class MapHandler():
    ...
    def load_map_from_occupancy_map(self, occupancy_map: np.ndarray, origin: np.ndarray, resolution: float) -> None:
    ...
    def _scale_occupancy_map(self, occupancy_map: np.ndarray, scale: float) -> np.ndarray:
    ...
    def get_inflated_map(self, inflation_radius: float=None) -> 'MapHandler':
        ...
        for i in range(self._occupancy_map.shape[0]):
            for j in range(self._occupancy_map.shape[1]):
                if abs(self._sdf_map[i, j]) <= inflation_radius and self._occupancy_map[i, j] == 0:
                    inflated_map[i, j] = self.OCC_INFLATION_VAL
                    
        new_map = MapHandler()
        new_map.load_map_from_occupancy_map(inflated_map, self.get_origin(), self.get_resolution())
    ...
    def merge_map(self, map: 'MapHandler') -> 'MapHandler':
        ...
        if self.get_origin()[0] != map.get_origin()[0] or self.get_origin()[1] != map.get_origin()[1]:
        ...
        # new map
        new_cmap2d = CMap2D()
        new_occupancy_map = self._saturate_occupancy_map(self._occupancy_map + filtered_map)
        new_cmap2d.from_array(new_occupancy_map, self.get_origin(), self.get_resolution())
    ...
    def get_map_size(self) -> np.ndarray:
        ...
        return (self._occupancy_map.shape[0] * resolution, self._occupancy_map.shape[1] * resolution)
    ...
    def get_obs_dist_array(self, pose_array: np.ndarray)->np.ndarray:
        ...
        # get distance to nearest obstacle
        sdf_dist_array = self._sdf_map[in_ij_coordinates_array[:,0], in_ij_coordinates_array[:,1]]
    ...
    def construct_dijkstra_map(self, goal_pose: np.ndarray, mask: np.ndarray=None, extra_costs=None, inv_value=None, connectedness: int=8):        
        ...
        dijkstra_map = self._cmap2d.dijkstra(goal_ij, mask, extra_costs, inv_value, connectedness)
        
        return dijkstra_map
    
    def compute_shortest_path_dijkstra(self, start_pose: np.ndarray, dijkstra_map: np.ndarray, connectedness: int=8)->Tuple:
    ...
    def compute_lidar_scan_map(self, pos: np.ndarray, angles: np.ndarray, ranges: np.ndarray)->np.ndarray:
        ...
        scan_map = self._cmap2d.lidar_visibility_map_ij(pos_ij, angles_float32, ranges_float32)
               
        return scan_map

    def get_scan_points_map(self, pos: np.ndarray, angles: np.ndarray, ranges: np.ndarray)->'MapHandler':
        ...
        scan_map = MapHandler()
        scan_map.load_map_from_occupancy_map(detected_occ, self.get_origin(), self.get_resolution())
        
        return scan_map
    
    
    def calc_relative_pose(self, source_xy: np.ndarray, source_yaw: float ,target_ij: np.ndarray)->np.ndarray:
        """
            Args:
    ...
    def calc_relative_pose_to_scan_points(self, source_xy: np.ndarray, source_yaw: float, FOV: np.ndarray=None) -> np.ndarray:
        """
        ...
        for i in range(occ.shape[0]):
            for j in range(occ.shape[1]):
                if occ[i, j] == self.OCC_MAX_VAL:
                    # calc relative pose
                    relative_pose = self.calc_relative_pose(source_xy, source_yaw, np.array([i, j]))
                    # check FOV
