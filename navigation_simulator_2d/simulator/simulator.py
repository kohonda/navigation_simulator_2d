...
import numpy as np
from navigation_simulator_2d.common import AgentState, RobotCommand, RobotObservation, MovingObstacle
...
class Simulator():
    ...
    def __init__(self)->None:
        ...
        # lidar settings
        self._scan_angles = np.linspace(self.SCAN_ANGLE_MIN, self.SCAN_ANGLE_MAX, self.NUM_SCAN_RAY)
    ...
    def reset(self, params: ParameterHandler)->MapHandler:
        ...
        if unknown_static_obs is None:
            ...
            self._static_obstacle_map.load_map_from_occupancy_map(white_map, origin, resolution)
        else:
            self._static_obstacle_map = MapHandler()
        ...
        # update obstacle map
        obstacle_map = self._generate_obstacle_map(self._static_obstacle_map, self._moving_obstacles)
    ...
    def set_robot_state_and_obstacles_and_traj(self, robot_state: AgentState, moving_obstacles: List[MovingObstacle], robot_traj: np.ndarray)->None:
    ...
    def _observe(self, robot_state: AgentState, min_dist_to_moving_obs: float)->RobotObservation:
        scan_points_map = self._merged_map.get_scan_points_map(robot_state.pos, self._scan_angles, self._scan_ranges)
        scan_relative_poses = scan_points_map.calc_relative_pose_to_scan_points(robot_state.pos, robot_state.yaw)
        is_collision = self._merged_map.check_collision(robot_state.pos, self.ROBOT_RADIUS)
        static_map_with_scan = self._static_map.merge_map(scan_points_map)

        return RobotObservation(state=robot_state, 
    ...
    def _update_robot_state(self, robot_state: AgentState, control_cmd: RobotCommand)->AgentState:
        
        rot_mat = np.array([[np.cos(robot_state.yaw), -np.sin(robot_state.yaw)], [np.sin(robot_state.yaw), np.cos(robot_state.yaw)]])
        new_pos = robot_state.pos + rot_mat.dot(control_cmd.linear_vel) * self.CONTROL_INTERVAL
    ...
    def _generate_obstacle_map(self, static_obstacle_map: MapHandler, moving_obstacles: List)->MapHandler:
        
        new_occ = static_obstacle_map.get_map_as_np('occupancy').copy()
        for obs in moving_obstacles:
            new_occ = obs.draw_2d(new_occ, static_obstacle_map.pose2index, static_obstacle_map.meter2pixel, static_obstacle_map.OCC_MAX_VAL)
            
        obstacle_map = MapHandler()
        obstacle_map.load_map_from_occupancy_map(new_occ, static_obstacle_map.get_origin(), static_obstacle_map.get_resolution())
